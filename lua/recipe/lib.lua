...
_G.__recipe_exit = function(id, code)
	...
	vim.notify(string.format("%s: %q %s", state, job.key, M.format_time(duration)), level)
	...
	if job.term == nil or (not job.recipe.keep_open and code == 0) then
		if job.term then
			if fn.bufloaded(job.term.buf) == 1 then
...
function M.execute(key, recipe)
	...
	local cmd = recipe.raw and recipe.cmd
		...
		if not job.running or recipe.restart then
			...
			if job.term then
				...
				if win ~= -1 then
					...
					if fn.bufloaded(job.term.buf) == 1 then
						api.nvim_buf_delete(job.term.buf, { force = true })
					end
					job.term = nil
				end
			end
